        ...
        <div class="main_container_calendar">
            ...
            if (isset($dayCount)) {
                for ($i = 1; $i <= $dayCount; $i++) {
                    ...
                    $subjectId = $_SESSION['homework']['subjectId'];
                    $studentId = $_SESSION['dataOfUser']['userId'];
                    $studentId = mysqli_fetch_all(mysqli_query($connect, "SELECT `studient_id` FROM `studients` WHERE `user_id` = '$studentId'"))[0][0];
                    ...
                    <form action="" method="post">
                        <div class="day">
                            ...
                            <?
                            if ($checkGrade = mysqli_fetch_all(mysqli_query($connect, "SELECT `grade` FROM `greades` WHERE `subjectId` = '$subjectId' AND `studentId` = '$studentId' AND `day` = '$i' AND `month` = '$month' AND `year` = '$year'")))
                                echo '<input type="submit" class="submitInput">';
                            ?>
                ...
                <div class="modal_box_content">
                    ...
                    <div class="gradeContainer">
                        ...
                        <div class="gradeSelect">
                            <input readonly class="radio">
                            <?
                            $dayOfGrade = $_POST['dayOfGrade'];
                            $grade = mysqli_fetch_all(mysqli_query($connect, "SELECT `grade` FROM `greades`
                            WHERE `subjectId` = '$subjectId' AND `studentId` = '$studentId' AND `day` = '$dayOfGrade' AND `month` = '$month' AND `year` = '$year'"))[0][0];
                            ?>
                            <span>
                                <?= $grade ?>
                            </span>
                    ...
                    <div class="bottom">
                        <?
                        $comment = mysqli_fetch_all(mysqli_query($connect, "SELECT `comment` FROM `greades`
                        WHERE `subjectId` = '$subjectId' AND `studentId` = '$studentId' AND `day` = '$dayOfGrade' AND `month` = '$month' AND `year` = '$year'"))[0][0];
                        if ($comment == NULL) {

                            echo '<input readonly  type="text" class="comment" maxlength="45" value="Преподаватель не оставил комментария">';
                        } else {
                            echo '   <input readonly  type="text" class="comment" maxlength="45" value="' . $comment . '">';
                        }
                        ?>
                    </div>
                </div>
